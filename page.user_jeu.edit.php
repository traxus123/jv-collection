<?php
	...
	if (count($_POST) > 0) {
		...
		*/
		if ($post_check) {
			$return = Jeu::u_update($_GET['id'], $user->id, $_POST['Etat']);
...
<section>
	...
	<?php
		...
		$jeu = new Jeu(Jeu::u_select($_GET['id']));
	?>
					...
					<td>
						<?php
							if (count($_POST) > 0) {
								...
								echo '<input name="Etat" placeholder="Entrez un Etat." style="width: 100%;" type="text" value="' . $jeu->u_etat() . '" />';
							}
						?>
					</td>
					<td>
